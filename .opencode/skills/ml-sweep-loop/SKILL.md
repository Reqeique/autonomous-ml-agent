...
1. **Monitor** - pull all runs from the shared W&B sweep (`SWEEP_WANDB_SWEEP_ID`), rank by BPC under budget, detect stagnation.
...
## Monitor
- Sweep id/entity/project come from env. The `opencode-ml-sweep` workflow sets DEDICATED vars (`SWEEP_WANDB_ENTITY`, `SWEEP_WANDB_PROJECT`, `SWEEP_WANDB_SWEEP_ID`) which point at the **GPU project** (`ml-playground-perpos-gpu` under the `-custom` entity). Use ONLY the `SWEEP_WANDB_*` names. Do NOT trust the repo-wide `WANDB_ENTITY`/`WANDB_PROJECT` vars (those are the TPU agent's `-bpc` project). Never read, set, or create the shared `WANDB_SWEEP_ID` repo variable - it is deliberately NOT set (a previous sweep loop leak confused the TPU training agent). The canonical baseline is `state/baseline.json`.
- Query via `wandb.Api()`:
  ```python
  import wandb, os
  ...
  project = os.environ.get("SWEEP_WANDB_PROJECT") or os.environ.get("WANDB_PROJECT")
   sweep_id = os.environ.get("SWEEP_WANDB_SWEEP_ID")
  api = wandb.Api()
...
4. **Create the successor sweep** programmatically:
   ...
   ```
   Update `state/sweep_state.json` -> `sweep_id` and the workflow repo variable `SWEEP_WANDB_SWEEP_ID` (via `gh variable set SWEEP_WANDB_SWEEP_ID --repo <owner>/autonomous-ml-agent -b <new_id>`), so the NEXT orchestrator push joins the successor sweep. Do NOT create or set `WANDB_SWEEP_ID` (shared var, deliberately absent). Also open a PR with the sweep_config.json edit.
...
- GitHub: `gh variable set SWEEP_WANDB_SWEEP_ID --repo <owner>/autonomous-ml-agent -b <id>`; `gh pr create` for fold/steer PRs. Never touch `WANDB_SWEEP_ID`.
...
- **Empty sweep after successor:** if `SWEEP_WANDB_SWEEP_ID` points at a finished sweep with no runs, treat as "create successor now" not "error".

## Definition of Done (one cycle)
- `state/sweep_state.json` updated with fresh best + stagnation status + notes.
- If steering: `sweep_config.json` narrowed + successor sweep id created + `SWEEP_WANDB_SWEEP_ID` variable updated + PR opened.
