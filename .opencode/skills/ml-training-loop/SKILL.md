...
- **Never** increase epochs / `limit_chars` / `block_size` / `batch_size` to lower loss. That is compute inflation: it burns the ~9 h TPU session budget and crowds out sweeps/ablations that could actually move the architecture.
- **The budget is LOCKED and FINAL.** The canonical kernel runs at exactly `limit_chars=10M, block_size=512, stride=256, batch_size=384` (see `state/baseline.json` → `committed_budget`). Do not touch these fields.
- **Your job is to EXHAUST/EXPLOIT the budget, not escape it.** Squeeze the LOWEST possible BPC *within* these fixed compute limits — via the novel PerPos architecture, data pipeline, LR schedule, regularization. Never inflate compute to cut BPC; that is the cheat.
...
- **Push rule:** a diff whose only effect is more compute (epochs↑, `limit_chars`↑, `block_size`↑, batch↑, decoder layers↑, dim_ff↑) with no efficiency argument is NOT a valid improvement — reject it, do not push it.
- **Decoder CAPS (anti-GPT-ification, HARD — enforced in the kernel's `coerce_config`):** `token_dec_layers <= 2`, `token_dec_dim_ff <= 256`, `chunk_dec_layers <= 8`, `chunk_dec_dim_ff <= 2048`. The token decoder is a SMALL upsampler by design; inflating decoder layers/FF turns PerPos into a generic GPT transformer and trivially lowers BPC while undermining the hierarchy. Never push a config over these caps — the kernel clamps them anyway and logs the adjustment.
...
1. Skip straight to **Monitor/Analyze**: `kaggle kernels status <slug>`, `kaggle kernels output <slug> -p /tmp/opencode/artifacts` to pull `/kaggle/working` artifacts **outside the repo**, query W&B API for the run metrics. NEVER pull artifacts into the working tree.
...
## Tools & Commands
- Kaggle CLI: `kaggle kernels push -p kernels/perpos-enwik8`, `kaggle kernels status <slug>`, `kaggle kernels output <slug> -p /tmp/opencode/artifacts` (ALWAYS outside the repo).
- **Git hygiene (HARD rule):** `artifacts/`, `*.weights`, `*.h5`, and `wandb/` are gitignored and MUST NEVER be committed. Never use `git add -f` or `git add artifacts/`. Kernel/run artifacts (weights, logs, wandb dumps) can be 100+ MB and will be rejected by GitHub's `GH001` large-file hook, failing the push. Only ever stage code/config/state JSON with explicit paths (e.g. `git add kernels/ state/ scripts/`).
...
The workflow drives the fallback chain (primary → fallback → last-resort) via a rate-limit
watchdog that kills a model stuck on repeated `429 Too Many Requests` and starts the next one.
The agent does NOT self-redispatch on rate limits — doing so would spawn a duplicate concurrent
run. If the model degrades mid-run, stop promptly and exit; the watchdog handles switching.

| Order | Model (workflow `model:` input) | Provider | Notes |
| --- | --- | --- | --- |
| 1 (primary) | `nvidia/z-ai/glm-5.2` | NVIDIA NIM | Uses `NIM_API_KEY` → `NVIDIA_API_KEY`. |
| 2 (fallback, free) | `opencode/deepseek-v4-flash-free` | OpenCode Zen | Free; data may be used to improve. |
| 3 (last resort) | `nvidia/minimaxai/minimax-m3` | NVIDIA NIM | Same key as #1. |

Do not invoke `scripts/retrigger_with_fallback.sh` — the workflow already handles fallback.
Use it only as a manual/offline tool.
